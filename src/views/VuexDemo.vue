      ...
      <v-flex lg6>
        <v-layout row wrap>
          <v-flex xs6 md6 lg6>
            <v-form>
              <h2>Szólánc</h2>
              <v-text-field v-model="NoteText" label="Szó:" required />
              <br>
              <v-btn color="info" block @click="$store.dispatch('addNote', NoteText); NoteText = '';">Feljegyzés hozzáadása</v-btn>
            </v-form>
          </v-flex>
          <v-flex v-for="(item, index) in $store.state.note.Notes" :key="index" xs3 md3 lg3>
            <v-card class="ma-2">
              <v-card-title>
                <div>
